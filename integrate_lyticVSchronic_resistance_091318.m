...
function [T,Y] = integrate_lyticVSchronic_resistance_091318(tend,K,gamma,eta,delta,beta,beta_max,lambda,f,k,d,stress_times,IC)
% integrate system
...
dIF = hollings(etaF*(1-fF),h,PF,S) - delta*IF - k(3)*s(t,stress_times)*IF;
...
dCF = gamma_stress(lambda*gamma,s(t,stress_times))*CF*(1-Btot/K) + delta*IF - hollings(etaT,h,PT,CF) - k(5)*s(t,stress_times)*CF;
    % growth              + infection                - cross infection        - antibiotic death
dLF = gamma*LF*(1-Btot/K) + hollings(etaF*fF,h,PF,S) - hollings(etaT,h,PT,LF) - k(6)*s(t,stress_times)*LF;
       % infection                     - eclipse     - antibiotic death
dIFT_T = hollings(etaF*(1-fF),h,PF,LT) - delta*IFT_T - k(7)*s(t,stress_times)*IFT_T;
       % infection                     - burst       + induce lysis                      - antibiotic death
dIFT_C = hollings(etaT*(1-fT),h,PT,CF) - delta*IFT_C + s(t,stress_times)*k_ind(13)*CFT_F - k(8)*s(t,stress_times)*IFT_C;
       % infection                     - burst       + induce lysis                      - antibiotic death
dIFT_F = hollings(etaT*(1-fT),h,PT,LF) - delta*IFT_F + s(t,stress_times)*k_ind(12)*LFT_F - k(9)*s(t,stress_times)*IFT_F;
       % growth                 + cross infection          - induce lysis            - antibiotic death
dLFT_T = gamma*LFT_T*(1-Btot/K) + hollings(etaF*fF,h,PF,LT) - s(t,stress_times)*LFT_T - k(10)*s(t,stress_times)*LFT_T;
       % growth                                                        + chronic     - induce lysis            - antibiotic death
dCFT_T = gamma_stress(lambda*gamma,s(t,stress_times))*CFT_T*(1-Btot/K) + delta*IFT_T - s(t,stress_times)*CFT_T - k(11)*s(t,stress_times)*CFT_T;
       % growth                 + cross infection           - induce lysis            - antibiotic death
dLFT_F = gamma*LFT_F*(1-Btot/K) + hollings(etaT*fT,h,PT,LF) - s(t,stress_times)*LFT_F - k(12)*s(t,stress_times)*LFT_F;
       % growth                                                        + cross infection           - induce lysis            - antibiotic death
dCFT_F = gamma_stress(lambda*gamma,s(t,stress_times))*CFT_F*(1-Btot/K) + hollings(etaT*fT,h,PT,CF) - s(t,stress_times)*CFT_F - k(13)*s(t,stress_times)*CFT_F;
...
dPF = beta_stress(betaF,beta_max,s(t,stress_times))*(CF+CFT_T+CFT_F) - hollings(etaF,h,PF,(S+LT)) - d*PF;

    % - burst    + lysis due to stress (1-k_ind=0 if vulnerable to antibiotics)
dIR = - delta*IR + s(t,stress_times)*((1-k_ind(4))*LT+(1-k_ind(10))*LFT_T+(1-k_ind(11))*CFT_T+(1-k_ind(12))*LFT_F+(1-k_ind(13))*CFT_F);

...
kappa_LHS = LHS_Call(0.99, kappa, 1.01, 0 ,runs,'unif');
...
LHSmatrix = [lambda_LHS etaT_LHS etaC_LHS kappa_LHS A_LHS k_LHS ...
...
for x=1:runs %Run solution x times choosing different values
    ...
    k_vector = LHSmatrix(x,4)*ones(length(IC)-3,1);
    Bdiff = @(T) (averageB(tend,K,gamma,[LHSmatrix(x,2),LHSmatrix(x,3)],...
                  LHSmatrix(x,7),[LHSmatrix(x,10),LHSmatrix(x,11)],LHSmatrix(x,12),...
                  LHSmatrix(x,1),[LHSmatrix(x,8),LHSmatrix(x,9)],k_vector,...
                  LHSmatrix(x,13),T,LHSmatrix(x,5),LHSmatrix(x,6),IC)-Bthresh)^2;
    
    % save averages
    Tstar_lhs(1,x) = fminsearch(Bdiff,T0,options);
...
%CALCULATE PRCC
alpha=0.01;
[prccB, signB, sign_labelB]=PRCC(LHSmatrix(1:81,:),Tstar_lhs(1:81),1,PRCC_var,alpha);
